...
if [ $# -eq 4 ]; then
    if [ $4 -ge 6 ] || [ $4 -le 0 ] ; then
        echo "Erreur : centrale inexistante"
        echo "Opération terminée en 0 secondes."
        exit 4
    fi
	centrale=$4
...
case "$2_$3" in
	...
	"hvb_comp")
		...
		grep -E "^${centrale};[^-;]*;[-];[^;]*;[^;]*;[-];" "$fichier"|tr '-' '0'|cut -d';' -f 2,7,8| ./codeC/exec >> "$outputfile"  
	...
	"hva_comp")
		...
		grep -E "^${centrale};[^;]*;[^-;]*;[-];[^;]*;[-];" "$fichier"|tr '-' '0'|cut -d';' -f 3,7,8|./codeC/exec >> "$outputfile"
		tr ';' ':' < "$outputfile"  > temp && mv temp "$outputfile" 
		cat $outputfile
		;;
	...
	"lv_comp")
		...
		grep -E "^${centrale};[^;]*;[^;]*;[^-;]*;[^;]*;[-];" "$fichier"|tr '-' '0'|cut -d';' -f 4,7,8|./codeC/exec  >> "$outputfile"
		tr ';' ':' < "$outputfile"  > temp && mv temp "$outputfile" 
		cat $outputfile
		;;
	...
	"lv_all")
		...
		grep -E "^${centrale};[^;]*;[^;]*;[0-9]+;[^;]*;[^;]*;" "$fichier" | tr '-' '0' | cut -d';' -f 4,7,8 |./codeC/exec  >> $lv_temp
